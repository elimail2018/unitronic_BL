...
namespace ParkingSystem
{
    ...
    class VIPTicket : Ticket
    {
        ...
        public VIPTicket(int? timeLimit, int cost, Dimensions dimensions, string lots, List<VehicleClassType> VehicleClassTypes)
        {
            _ticketType = "vip";
        ...
        {
            if (!this.VehicleClassTypes.Contains(carClass)) return null;//car class is not valid for this ticket
            return this._parkingSpots.Find(p => p.IsAvailable && p.SpotLocation >= 11 && p.SpotLocation <= 30);
        }
    }
}
